...
def _arms_to_contrasts(study_id: str, arms: List[Dict], ext: Dict) -> List[Dict]:
    ...
    For k arms, generates k*(k-1)/2 contrasts.
    Uses the first arm as the reference for consistency.
    """
    import math
    contrasts = []
    k = len(arms)
    ...
    for i in range(k):
        for j in range(i + 1, k):
            a1 = arms[i]
            a2 = arms[j]
            t1 = a1.get("treatment", f"Arm_{i}")
            t2 = a2.get("treatment", f"Arm_{j}")

            # If TE/seTE provided directly in arm comparison
            if "TE" in a1 and "TE" in a2:
                te = float(a2["TE"]) - float(a1["TE"])
                se = math.sqrt(float(a1.get("seTE", 0))**2 + float(a2.get("seTE", 0))**2)
            # Compute SMD from mean/sd/n
            ...
            # Compute log-RR from events/total
...
def run_nma(contrasts: List[Dict[str, Any]],
    ...
    # Convert contrasts to CSV string
    csv_buffer = io.StringIO()
    writer = csv.DictWriter(csv_buffer, fieldnames=["studlab", "treat1", "treat2", "TE", "seTE"])
    writer.writeheader()
    ...
    r_script = template.replace("{{output_dir}}", output_dir.replace("\\", "/"))
